# -*- coding: utf-8 -*-
'''Elipse Plant Manager - EPM Dataset Analysis - module example
Copyright (C) 2018 Elipse Software.
Distributed under the MIT License.
(See accompanying file LICENSE.txt or copy at http://opensource.org/licenses/MIT)
'''

print('*** Importing MyModule - EPM-Analysis mini course ***')
import numpy as np
...
def movingAvgFilter( xepmo, o ):
	xepm = xepmo.copy() # copia os dados para não alterar a variável do console
...
# Retorna a string completa da palavra se p=-1(padrão), caso contrario um boleano (False|True) da posição solicitada
def int2bit(n, p = -1):
    if not n:
        return '0'
    s = bin(int(n))
    s = s.lstrip('-0b') # remove zero e sinal
    s_1 = ''
    ...
    if p < 0:
        ...
        if p < len(s_1):
            return bool(int(s_1[p]))
        else:
            return False

# Exportar para arquivo CSV
def export2csv( epmData, fileName ):
...
def savePdfReport( fileName, attImage ):
    # ATENÇÃO: Necessário ter instalado o módulo reportlab
    ...
    story = []
    story.append(Paragraph("EPM Data Analysis Report",styleH))
    story.append(Paragraph("Relatório gerado a partir do EPM Studio - Dataset Analysis", styleN))
    story.append(Paragraph(" ", styleN))
    im = Image(attImage, width=4*inch, height=3.02*inch)
    im.hAlign = 'CENTER'
    story.append(im)
    c = Canvas( fileName )
...
# Função que conta o numero de inversões de sentido
def invCount( a ):
    d = a[1:] - a[:-1] # delta
    n = 0
...
# Retorna um vetor com n elementos gerados a partir da soma dos valores pregressos com o corrente
def fibonacci(n):
    fib = np.arange(1)
    a = 0
    b = 1
    while a < n:
        fib, a, b = np.hstack(( fib, b )), b, a+b
    return fib

# Retorna uma matriz 4xN com informações das colunas de uma matriz NxM. As colunas da matriz retornada são: min, max, mean e std
def colStats( A ):
    n,m = np.shape(A)
    AMcol = np.array( (A[:,0].min(), A[:,0].max(), A[:,0].mean(), A[:,0].std()) )
    for i in range(1, m):
        AMcol = np.vstack( (AMcol, (A[:, i].min(), A[:, i].max(), A[:, i].mean(), A[:,i].std())))
    return AMcol
...
def amplitudeEstimation( x, w = 0.5 ):
    ...
    dx = abs( x - xf )
    # Estima a amplitude baseada na amplitude media e o desvio padrao (2 desvios)
...
# Grafico de histograma informando dados do EPM
def histPlot( xepm, ei = -1, es = -1 ):
    nd = len(xepm) # numero de pontos
    ni = int(np.sqrt(nd)) # numero de intervalos
    xo = xepm['Value'].copy()
    mu = xo.mean() # media da distribuicao
    sigma = xo.std() # desvio padrao
    if ei < 0:
       ei = xo.min()
    if es < 0:
       es = xo.max()
    # Capacidade de produzir dentro das especificacoes e reprodutibilidade nas medidas (processo centrado nas especificações de Engenharia)
    ...
    n, bins, patches = plt.hist(xo, ni, density=1, facecolor='green', alpha=0.5)
    ...
    figManager.set_window_title('Histogram and Cp/Cpk')
    figManager.window.state('zoomed')
    plt.show()

# Gráfico com informações de CEP (Controle Estatístico de Processo)
def cep( xepm, ei = None, es = None ):
    n = len(xepm) # número de pontos
    x = xepm['Value'].copy()
    t = xepm['Timestamp'].copy()
    mu = x.mean() # média da distribuição
    ...
    # Histograma dos dados, envelope com a curva normal e o Box Plot
    pOut = 1.5
    fig = plt.figure(1, figsize=(10.5,10.5))
    axHist = fig.add_subplot(5, 1, 1)
    m, bins, patches = axHist.hist(x, binsNum, density=1, facecolor='green', alpha=0.5)
    axHist.set_aspect(1.)
    y = stats.norm.pdf(bins, x.mean(), x.std())
    plt.plot(bins, y, 'r--')
    ...
    # *** Cálculo das demais informações estatísticas ***
    # Remoção dos dados referentes às causas especiais (outliers) -  less than (Q1 - pOut * IQR)  OR  greater than (Q3 + pOut + IQR)
    ...
    posOut = np.argwhere(vecOut).ravel() # vetor com os índices das posições com dados válidos (não são causas especiais)
    ...
    # Cálculo dos limites de controle (valores)
    sec2min = np.vectorize(lambda x: int(x/60. + 0.5))
    td2sec = np.vectorize(lambda dt: dt.total_seconds())
    if len(x) == len(x1):   # outra forma de verificar seria:  np.any(posOut[1:]-posOut[:-1] >1) -> Se for verdadeiro é pq dados foram removidos
    ...
    # Gráfico Série temporal dos limites de controle
    ...
    figManager = plt.get_current_fig_manager()
    figManager.set_window_title('Statistical Process Control')
    figManager.window.state('zoomed')
    plt.show()
...
# Período de avaliação padrão de 6 h - primeiro timestamp serve de referencia para toda pesquisa (iniciar a 00:00:00 de algum dia e terminar pelo menos Tini + 6h)
# Avaliação é feita em base diária
def minFromPeriod( epmData, pHours = 6 ):
    ...
    evalPeriod = datetime.timedelta(hours = pHours) # período de avaliação - busca dos valores mínimos
    nextPeriod = iniPeriod + evalPeriod
    # Pesquisa diária segundo base horaria definida em iniPeriod ate evalPeriod (horas)
    minsPeriod = []
    for i in range(totDays+1):
        ...
        # Atualiza os períodos para o próximo dia
...
def percentTimeIn(epmData, nodes = -1):
    ...
    intervNum = np.size(nodes)+1 # [antes - intervalos - depois] (nodes padrão utilizou o min/max, logo não terá valores fora deste intervalo)
...
def rmNanAndOutliers(epmData, sd = 3):
    y = epmData['Value']
    t = epmData['Timestamp']
    # Remove os valores Nan de y e os correspondentes timestamps
    nanPos = np.argwhere(np.isnan(y)) # retorna os índices da matriz que são Nan
    y = np.delete(y,nanPos) # deleta as os valores das posições que contem nan
    t = np.delete(t,nanPos)
    # Remove pontos que estão sd desvios além da media
    s3 = np.floor(sd * y.std())
    smin = y.mean() - s3
    smax = y.mean() + s3
    outPos = np.argwhere(y<smin) # retorna os índices da matriz que sao menores que smin
    y = np.delete(y,outPos) # deleta as os valores das posições outPos
    t = np.delete(t,outPos)
    outPos = np.argwhere(y>smax) # retorna os índices da matriz que são maiores que smax
    y = np.delete(y,outPos) # deleta as os valores das posições outPos
    t = np.delete(t,outPos)
    res = vec2epm(t,y)
    return res

# Remove dados Nan e Outliers baseado no desvio padrão e retorna vetores t e y
def rmNanAndOutliers2(epmData, sd = 3):
    y = epmData['Value']
    t = epmData['Timestamp']
    # Remove os valores Nan de y e os correspondentes timestamps
    nanPos = np.argwhere(np.isnan(y)) # retorna os índices da matriz que são Nan
    y = np.delete(y,nanPos) # deleta as os valores das posições que contem nan
    t = np.delete(t,nanPos)
    # Remove pontos que estão sqrt(sd) além da media
    s3 = np.floor(sd * np.sqrt(y.std()))
    smin = y.mean() - s3
    smax = y.mean() + s3
    outPos = np.argwhere(y<smin) # retorna os índices da matriz que são menores que smin
    y = np.delete(y,outPos) # deleta as os valores das posições outPos
    t = np.delete(t,outPos)
    outPos = np.argwhere(y>smax) # retorna os índices da matriz que são maiores que smax
    y = np.delete(y,outPos) # deleta as os valores das posições outPos
...
# Retorna um objeto de dados a partir de um vetor de tempo e outro de dados, as qualidades são sempre boas
def vec2epm(t, y):
    """ Parametros
...
# Geração do perfil diário de uma variável - retorna uma matriz para plotar com matplotlib.pylab.plot (dados devem fechar em dia completo)
# Os dados devem estar interpolados (igualmente espaçados).
def dailyProfile3D( epmData, sampling = 30, pHours = 24):
    """
    epmData: dados do EPM
    sampling: amostragem da interpolação - informada em minutos
    pHours: intervalo para geração do perfil
    """
    iniPeriod = epmData['Timestamp'][0]
    endPeriod = epmData['Timestamp'][-1]
    dTotal = endPeriod - iniPeriod
    totDays = dTotal.days # número de dias a serem avaliados
    evalPeriod = datetime.timedelta(hours = pHours) # período de avaliação - busca dos valores mínimos
    nextPeriod = iniPeriod + evalPeriod
    # Pesquisa diária segundo base horaria definida em iniPeriod ate evalPeriod (horas)
    profileList = []
...
def pingMachine( ip ):
    """
    ip: IP da maquina ou o nome - deve ser uma String
    """
    import os, re
    res = 0 # não conseguiu
    cmd = "ping -4 " + ip
    r = "".join(os.popen(cmd).readlines())
    if re.search ("Lost = 0", r):
       res = 1
    return res
...
# Resíduos para estimar parâmetros da curva de Pot com o Vento estimando tbm Pnominal
def residualsSPPn(par, x, y):
    return powerFitPn(par, x) - y

# Análise de dados Energia Eólica - Wind Power Generator - versão com Spline
def windPower(epmWindSpeed, epmPower, Pnominal, csvFileRef):
    ...
    # Remove potências superiores à potência nominal informada
    pPos = np.argwhere(powerData > Pnominal)
    speed = np.delete(speed,pPos)
    power = np.delete(power,pPos)
    # Remove valores inferiores a 4 m/s de velocidade de vento
    pPos = np.argwhere(speed < 4.)
    speed = np.delete(speed,pPos)
    power = np.delete(power,pPos)
    # Gráfico scatter (dispersão) da potência com a velocidade do vento
    ...
    speedRef, powerRef = readFromCsv(csvFileRef)
    ...
    # Calculo da área entre as curvas de referencia e a obtida através dos dados medidos -> Energia que não esta sendo gerada
...
# Histograma das velocidades dos ventos
def windHistogram(epmWindSpeed):
    speedData = epmWindSpeed['Value'].copy()
    nd = len(speedData) # número de pontos
    ni = int(np.sqrt(nd)) # número de intervalos
    mu = speedData.mean() # média da distribuição
    sigma = speedData.std() # desvio padrão
    n, bins, patches = plt.hist(speedData, ni, density=1, facecolor='green', alpha=0.5)
    y = stats.norm.pdf(bins, mu, sigma) # envelope da curva normal
    plt.plot(bins, y, 'r--')
    title = r'Histograma: $\mu='
    title += str(round(mu,2))
    title +=r'100$, $\sigma='
    title += str(round(sigma,2))
    title += '$'
    plt.title(title)
    font = {'family' : 'serif',
        'color'  : 'darkred',
...
# Determinação dos valores médios de potencia para cada valor de velocidade
...
# Funções auxiliares para filtrar de sinais
def lFilter_zi( b, a ):
    # Determinação do estado Zi dos parâmetros do filtro
    n = max(len(a),len(b))
...
# Filtro nos dois sentidos para remover delay - apenas para vetores
def filtFilt( b, a, x ):
    ntaps = max(len(a),len(b))
    edge = ntaps * 3
    if x.ndim != 1:
        raise ValueError, "Apenas para vetor"
    # x deve ser maior que o extremo
    if x.size < edge:
        raise ValueError, "Vetor de entrada deve ser maior que 3 * max(len(a),len(b)."
    if len(a) < ntaps:
